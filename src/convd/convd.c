...
static const char LIBCONVD_NAME[] = "libconvd";
static const char LIBCONVD_VERSION[] = "1.0.0";
...
static int strcmp_caseign(const char *Astr, const char *Bstr)
{
    #ifdef _MSC_VER
...
int strcpy_safe(char *dstbuf, size_t dstbufsize, size_t dstoffset, const char *source, size_t sourcelen)
{
...
{
    if (_libname) {
        *_libname = LIBCONVD_NAME;
    }
    return LIBCONVD_VERSION;
}
...
{
    char headbuf[128];
    int headlen = snprintf(headbuf, sizeof(headbuf), "<?xml version=\"%s\" encoding=\"%s\"?>", xmlhead->version, xmlhead->encoding);

    switch (xmlhead->bom) {
    case UCS_NONE_BOM:
        if (strcpy_safe(output->bufp, output->blen, 0, headbuf, headlen)) {
            return headlen;
        }
        return 0;
    ...
    case UCS_UTF8_BOM:
        ...
        if (strcpy_safe(output->bufp, output->blen, 3, headbuf, headlen)) {
            return headlen + 3;
        }
        return 0;
...
{
    if (header->blen >= 2) {
        uint8_t b0 = (uint8_t)header->bufp[0];
        uint8_t b1 = (uint8_t)header->bufp[1];
        ...
        if (b0 == 0xFF && b1 == 0xFE) {
            if (header->blen >= 4) {
                uint8_t b2 = (uint8_t)header->bufp[2];
                uint8_t b3 = (uint8_t)header->bufp[3];
                if (b2 == 0x00 && b3 == 0x00) {
                    return UCS_4LE_BOM;
                }
            }
...
int XML_text_parse_head(const conv_buf_t *xmltext, conv_xmlhead_t *xmlhead)
{
    ...
    case UCS_2BE_BOM:
        ...
        while(offset + 1 < (int)xmltext->blen) {
            if (headlen < sizeof(headbuf) - 1) {
                // 避免不安全的类型转换
                sb2 be16;
                memcpy(&be16, &xmltext->bufp[offset], sizeof(be16));
                headbuf[headlen] = (char) BO_betoh16(be16);
                offset += 2;
                if (headbuf[headlen++] == '>') {
                    headbuf[headlen] = '\0';
                    found = 1;
    ...
    case UCS_2LE_BOM:
        ...
        while(offset + 1 < (int)xmltext->blen) {
            if (headlen < sizeof(headbuf) - 1) {
                sb2 le16;
                memcpy(&le16, &xmltext->bufp[offset], sizeof(le16));
                headbuf[headlen] = (char) BO_letoh16(le16);
                offset += 2;
                if (headbuf[headlen++] == '>') {
                    headbuf[headlen] = '\0';
                    found = 1;
    ...
    case UCS_4BE_BOM:
        ...
        while(offset + 3 < (int)xmltext->blen) {
            if (headlen < sizeof(headbuf) - 1) {
                sb4 be32;
                memcpy(&be32, &xmltext->bufp[offset], sizeof(be32));
                headbuf[headlen] = (char) BO_betoh32(be32);
    ...
    case UCS_4LE_BOM:
        ...
        while(offset + 3 < (int)xmltext->blen) {
            if (headlen < sizeof(headbuf) - 1) {
                sb4 le32;
                memcpy(&le32, &xmltext->bufp[offset], sizeof(le32));
                headbuf[headlen] = (char) BO_letoh32(le32);
    ...
    if (found && headlen > 20 && cstr_startswith(headbuf, headlen, "<?xml ", 6) &&
...
static void ConvdCleanupCallback(void *cvd)
{
...
static int ucs_file_read_open(convd_t cvd, const char *pathfile, ub4 linesizemax, convpos_t *_cpos)
{
    ...
    if (hf != filehandle_invalid) {
        ...
        cpos = (convpos_t) mem_alloc(sizeof(conv_position_t) + (5 * linesizemax) * sizeof(char));
...
int convd_create(const char *fromcode, const char *tocode, CONVD_SUFFIX_MODE suffix, convd_t *outcvd)
{
    ...
    cvd = (conv_descriptor_t *) refcobj_new(0, sizeof(conv_descriptor_t) + fromlen + tolen + sizeof(char) * 12, ConvdCleanupCallback);
...
{
    refcobj_dec((void**)pcvd);
}


convd_t convd_retain(convd_t *cvd)
{
    return (convd_t)refcobj_inc((void**)cvd);
}
...
size_t convd_conv_xmltext(convd_t cvd, conv_buf_t *input, conv_buf_t *output)
{
    ...
    if (! memcmp(convd_tocode(cvd, 0), "UTF-8", 5) ||
        ...
        // ansi == utf8
        xmlhead.bom = 0;
        strcpy_safe(xmlhead.encoding, sizeof(xmlhead.encoding), 0, convd_tocode(cvd, 0), cstr_length(convd_tocode(cvd, 0), sizeof(xmlhead.encoding) - 1));
...
int convd_conv_xmlfile(convd_t cvd, const char *xmlfilein, const char *xmlfileout, ub4 linesizemax, int addbom, ub8 *outfilesize)
{
    ...
    if (headofflen > 0) {
        if (! memcmp(convd_tocode(cvd, 0), "UTF-8", 5) ||
            ...
            // ansi == utf8
            xmlhead.bom = 0;
            strcpy_safe(xmlhead.encoding, sizeof(xmlhead.encoding), 0, tocode, tocodelen);
